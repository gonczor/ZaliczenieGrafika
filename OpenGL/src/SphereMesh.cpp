...
void SphereMesh::addVertecies()
{
	const GLfloat PI = glm::pi<GLfloat>();

	for (unsigned int ring = 0; ring < m_size; ++ring)
	{
		GLfloat lontitiude = map(static_cast<GLfloat>(ring), 0.0f, static_cast<GLfloat>(m_size) + 1, -PI, PI);
		for (unsigned int point = 0; point < m_sectors; ++point)
		{
			GLfloat latituide = map(static_cast<GLfloat>(point), 0.0f, static_cast<GLfloat>(m_sectors) + 1, 0.0f, 2.0f * PI);
